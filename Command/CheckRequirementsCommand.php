...
class CheckRequirementsCommand extends AbstractInstallCommand
{
    ...
    {
        $this
            ->setName('dos:install:check-requirements')
            ->setDescription('Checks if all Project requirements are satisfied.')
            ->setHelp(<<<EOT
...
EOT
    ...
    {
        $fulfilled = true;
        $requirements = $this->get('dos.requirements');
        ...
        if (!$fulfilled) {
            ...
            $output->writeln('<info>Success! Your system can run Project properly.</info>');
        }
    }
}
